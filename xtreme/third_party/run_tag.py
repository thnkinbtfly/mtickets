...
import numpy as np
import torch
from util.common import total_lang2id, set_uriel
...
from util.bert_with_mask import (
  BertForTokenClassificationWithMask, BertConfigWithMask
)
...
MODEL_CLASSES = {
  ...
  "bert_with_mask": (BertConfigWithMask, BertForTokenClassificationWithMask, BertTokenizer),
...
def load(model_type, checkpoint, orig_model, config, prune_model=False):
  _, model_class, _ = MODEL_CLASSES[model_type]
  if prune_model:
    model = model_class.from_pretrained(orig_model, config=config)
    model.base_model.prune()
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1):
  ...
  tokenzier_name = args.tokenizer_name or args.model_name_or_path
  cached_features_file = os.path.join(args.data_dir, "cached_{}_{}_{}_{}".format(mode, lang,
    list(filter(None, tokenzier_name.split("/"))).pop(),
...
def main():
  ...
  parser.add_argument("--adaptive_least_seen", default=None, type=int,
                      help="Assure at least seeing this amount of samples")
  ...
  parser.add_argument("--use_gelu_new", action="store_true",
                      help="Use the activation used in pretraining")
  parser.add_argument("--total_languages", type=int, default=103,
                      help="Scale the mask")
  parser.add_argument("--mask_method", type=str, default='linear',
                      help="Masking method")
  parser.add_argument("--mask_val", type=int, default=1,
                      help="Masking val for constant masking")
  parser.add_argument("--prune_model", default=False, action='store_true',
                      help="Whether or not to prune the model before training")
  ...
  config_kwargs = {}
  if args.use_gelu_new:
    config_kwargs = {"hidden_act": "gelu_new"}
  config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
                      num_labels=num_labels,
                      cache_dir=args.cache_dir if args.cache_dir else None, **config_kwargs)
  config.mask_method = args.mask_method
  config.mask_val = args.mask_val
  config.total_languages = args.total_languages
  ...
  if args.do_train:
    ...
    if args.adaptive_least_seen:
      if len(train_dataset) * args.num_train_epochs < args.adaptive_least_seen:
        import math
        args.num_train_epochs = math.ceil(args.adaptive_least_seen / len(train_dataset))
        logger.info("num_train_epochs is set to {}".format(args.num_train_epochs))

    assert len(args.train_langs.split(',')) == 1
    model.config.masked_language = args.train_langs
    print("Masked language: {}".format(model.config.masked_language))
  ...
  if args.do_eval and args.local_rank in [-1, 0]:
    ...
    for checkpoint in checkpoints:
      ...
      model = load(args.model_type, checkpoint, args.model_name_or_path, config, args.prune_model)
      model.to(args.device)
      result, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev",
                           lang=args.train_langs, lang2id=lang2id,
                           prefix="eval", train_lang=args.train_langs)
  ...
  if args.do_predict and args.local_rank in [-1, 0]:
    ...
    if args.save_only_best_checkpoint:
      ...
      model = load(args.model_type, best_checkpoint, args.model_name_or_path, config, args.prune_model)
      model.to(args.device)

    output_test_results_file = os.path.join(args.output_dir, "test_results.txt")
    with open(output_test_results_file, "a") as result_writer:
      for lang in args.predict_langs.split(','):
        model.config.masked_language = lang
        print("Masked language: {}".format(model.config.masked_language))

        infile = os.path.join(args.data_dir, lang, 'test.{}'.format(args.tokenizer_name or args.model_name_or_path))
        ...
        result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="test", lang=lang, lang2id=lang2id,
                                       train_lang=args.train_langs, prefix="predict")

        # Save results
        result_writer.write("=====================\nlanguage={}\n".format(lang))
        for key in sorted(result.keys()):
          result_writer.write("{} = {}\n".format(key, str(result[key])))
          # avgmeter.update(result[key], lang, key)
        # Save predictions
  ...
  if args.do_predict_dev and args.local_rank in [-1, 0]:
    ...
    with open(output_test_results_file, "w") as result_writer:
      for lang in args.predict_langs.split(','):
        ...
        infile = os.path.join(args.data_dir, lang, 'dev.{}'.format(args.tokenizer_name or args.model_name_or_path))
        if not os.path.exists(infile):
          logger.info("Language {} does not exist".format(lang))
          continue
        result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev", lang=lang, lang2id=lang2id,
                                       train_lang=args.train_langs, prefix="predict_dev")
